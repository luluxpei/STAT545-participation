...
gapminder$continent %>%
  str() # integer = 3 (gapminder dataset starts with observations from Asia)
```
...
```{r}
gapminder$country %>%
  nlevels() # 142 levels (countries)

countries = c("Canada", "Mexico", "China", "Australia", "France")

filtered <- gapminder %>%
  filter(country %in% countries)

filtered$country %>%
  nlevels() # still preserves 142 levels 
```
...
```{r}
h_gap_dropped <- filtered %>% 
  droplevels()

h_gap_dropped$country %>%
  nlevels() # 5 levels for country factor
```
...
This might be useful if you are preparing a report for say, the state of affairs in Africa (`fct_relevel()` function).
...
Let's recode `Oceania` and the `Americas` in the graph above as abbreviations `OCN` and `AME` respectively using the function `fct_recode()`.
